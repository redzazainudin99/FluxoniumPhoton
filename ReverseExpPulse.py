...
class ReverseExpShape(PulseShape):
    def __init__(self):
        super().__init__()
    ...
    def model_func(self, time):
        decay_rate = self.duration / np.log(1000)
        waveform = (1/30) * self.amplitude * np.exp(-1 * (-time/decay_rate))
        print(type(waveform))
        print(time)
...
class ReverseExp(Pulse):
    def __init__(
        ...
        super().__init__()
        self.pulse_shape = ReverseExpShape()
        self.params = {
            "amplitude" : amplitude,
            "duration" : duration,
            "zero_end" : zero_end
...
seq.add(ReverseExp(amplitude= 0.2, duration = 2000, zero_end=False), readout_port)
