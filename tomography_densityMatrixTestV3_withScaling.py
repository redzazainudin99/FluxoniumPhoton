...
data = DataDict(
    ...
    p_shot_vacuum=dict(axes=["shot"]),
    q_shot = dict(axes=["shot"]),
    q_shot_vacuum = dict(axes=["shot"]),
)
data.validate()

# we test for the density matrix of a specific state

sequence_p = photon_sequence_resetted_wScaling(init_state='1', phase= 0 )
sequence_q = photon_sequence_resetted_wScaling(init_state='1', phase=np.pi)
