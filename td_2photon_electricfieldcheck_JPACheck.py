...
def photon_sequence (virtualz, phase, draw_end = False):
    ...
    if virtualz ==True:
        sequence.add(VirtualZ(np.pi), ge_drive_port)
...
#     # sequence.add(Delay(480), JPA_port, copy=False)   
#     # sequence.add(Delay(480), digi_port, copy=False)  
...
#     if phase == 0:
#         JPA_pulse.params['amplitude'] = JPA_pulse.params['amplitude']
#         sequence.add(JPA_pulse, JPA_port, copy=False)  
#     elif phase ==2*np.pi:
#         JPA_pulse.params['amplitude'] = - JPA_pulse.params['amplitude']
#         sequence.add(JPA_pulse, JPA_port, copy=False)  
...
# seq = photon_sequence(virtualz=False, phase = 2*np.pi, draw_end=True)
# raise SystemError
...
####GF PULSE PARAMS###
gf_pi.params['amplitude'] = 0.7
gf_pi.params['duration'] = drive_pulse_duration
