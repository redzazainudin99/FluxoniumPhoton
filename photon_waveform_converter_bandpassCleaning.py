...
# def bandpass_filter_truncated(freqs, signal, f_low, f_high):

#     freqs = np.array(freqs)
#     signal = np.array(signal)

#     # Mask for frequencies within [f_low, f_high]
#     band_mask = (freqs >= f_low) & (freqs <= f_high)

#     # Truncate both arrays
#     freqs_band = freqs[band_mask]
#     signal_band = signal[band_mask]

#     return freqs_band, signal_band
...
fft_signal = bandpass_filter(fft_time, fft_signal, readout_freq*1e9 - 0.01e9, readout_freq*1e9 + 0.01e9)
...
plt.figure()
plt.plot(time, phase)
plt.title("Unwrapped Phase (should be flat if successful)")
plt.xlabel("Time")
...
phase_fit = np.polyval(coeffs, time)

# Remove the linear trend
reconstructed_signal *= np.exp(-1j * (phase_fit -np.pi * 1.3))

# Plot new unwrapped phase
plt.plot(time, np.unwrap(np.angle(reconstructed_signal)))
plt.title("Phase after Linear Trend Removal")
plt.xlabel("Time")
plt.ylabel("Phase (rad)")
plt.tight_layout()
if show_plots:
...
def load_sequence_append(waveform_appended, append_port,sequence: Sequence, cycles: int):
    ...
    if ge_drive_port in sequence.port_list:
        ...
        ge_drive = waveform_dict[ge_drive_port.name].real
        # plt.plot(i);plt.show()
