...
delay = Variable("delay", 10*np.arange(30) + 10, "ns")
...
# sequence.add(Acquire(duration = len(reversed_photon)),  readout_port, copy = False)   #reverse exp

#absorption pulse
sequence.add(Delay(duration = delay), ge_drive_port, copy= False )  
...
with DDH5Writer(data, data_path, name=measurement_name) as writer:
    ...
    for update_command in tqdm(variables.update_command_list):
        ...
        # load_sequence_append( reversed_photon,readout_port,sequence, cycles=50000)
        writer.add_data(
