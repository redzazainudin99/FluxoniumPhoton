...
delayarray = 20*np.arange(100) + 10
delay_val = Variable("delay",delayarray, "ns")
amplitude = Variable("amplitude",np.linspace(0,1, 40), "V")
# delay = Variable("delay",[10,20], "ns")
# duration = Variable("duration",100*np.arange(50) + 4000, "ns")
variables = Variables([delay_val,amplitude])
...
revExp_sequence.add(ReverseExp(amplitude=0.05, duration=7000),readout_port, copy=False )
revExp_sequence.add(Delay(500), readout_port)
